export const quotes = [
    // A Christmas Carol
    {
        text: "Solitary as an oyster",
        source: ["A Christmas Carol", "Scrooge", "Stave 1"]
    },
    {
        text: "Hard and sharp as flint, from which no steel had ever struck out generous fire",
        source: ["A Christmas Carol", "Scrooge", "Stave 1"]
    },
    {
        text: "a squeezing, wrenching, grasping, scraping, clutching, covetous old sinner",
        source: ["A Christmas Carol", "Scrooge", "Stave 1"]
    },
    {
        text: "Darkness is cheap and Scrooge liked it",
        source: ["A Christmas Carol", "Scrooge", "Stave 1"]
    },
    {
        text: "Bah humbug",
        source: ["A Christmas Carol", "Scrooge", "Stave 1"]
    },
    {
        text: "Are there no prisons",
        source: ["A Christmas Carol", "Scrooge to Portly Gentleman", "Stave 1"]
    },
    {
        text: "decrease the surplus population",
        source: ["A Christmas Carol", "Scrooge to Portly Gentleman", "Stave 1"]
    },
    {
        text: "He was all in a glow; his face was ruddy and handsome; his eyes sparkled",
        source: ["A Christmas Carol", "Fred", "Stave 1"]
    },
    {
        text: "The chain he drew was clasped about his middle",
        source: ["A Christmas Carol", "Marley", "Stave 1"]
    },
    {
        text: "I wear the chain I forged in life, I made it link by link, and yard by yard",
        source: ["A Christmas Carol", "Marley", "Stave 1"]
    },
    {
        text: "Mankind was my business",
        source: ["A Christmas Carol", "Marley", "Stave 1"]
    },
    {
        text: "Like a child; yet not so like a child as like an old man",
        source: ["A Christmas Carol", "Ghost of Christmas Past", "Stave 2"]
    },
    {
        text: "Another idol has displaced meâ€¦a golden one",
        source: ["A Christmas Carol", "Belle", "Stave 2"]
    },
    {
        text: "The master-passion, Gain, engrosses you",
        source: ["A Christmas Carol", "Belle", "Stave 2"]
    },
    {
        text: "A jolly giant, glorious to see",
        source: ["A Christmas Carol", "Ghost of Christmas Present", "Stave 3"]
    },
    {
        text: "as good as gold",
        source: ["A Christmas Carol", "Cratchit Family", "Stave 3"]
    },
    {
        text: "yellow, meagre, ragged, scowling, wolfish",
        source: ["A Christmas Carol", "Ignorance and Want", "Stave 3"]
    },
    {
        text: "where angels might have sat enthroned, devils lurked",
        source: ["A Christmas Carol", "Ignorance and Want", "Stave 3"]
    },
    {
        text: "They are Man's",
        source: ["A Christmas Carol", "Ignorance and Want", "Stave 3"]
    },
    {
        text: "it seemed to scatter gloom and mystery",
        source: ["A Christmas Carol", "Ghost of Christmas Yet to Come", "Stave 4"]
    },
    {
        text: "I am as happy as an angel",
        source: ["A Christmas Carol", "Scrooge's Transformation", "Stave 5"]
    },
    {
        text: "I will honour Christmas in my heart",
        source: ["A Christmas Carol", "Scrooge's Transformation", "Stave 5"]
    },
    {
        text: "The cold within him froze his old features",
        source: ["A Christmas Carol", "Narrator", "Stave 1"]
    },
    {
        text: "I think he loses some pleasant moments, which could do him no harm",
        source: ["A Christmas Carol", "Fred", "Stave 1"]
    },
    {
        text: "A solitary child, neglected by his friends, is left there still",
        source: ["A Christmas Carol", "Ghost of Christmas Past", "Stave 2"]
    },
    {
        text: "Remove me from this place",
        source: ["A Christmas Carol", "Scrooge", "Stave 2"]
    },
    {
        text: "They shone in every part of the dance like moons",
        source: ["A Christmas Carol", "Fezziwig's Party", "Stave 2"]
    },
    {
        text: "Wonderful party, wonderful games, wonderful unanimity",
        source: ["A Christmas Carol", "Fezziwig's Party", "Stave 2"]
    },
    {
        text: "These young Cratchits danced about the table",
        source: ["A Christmas Carol", "Cratchit Family", "Stave 3"]
    },
    {
        text: "Nobody said or thought it was at all a small pudding for a large family",
        source: ["A Christmas Carol", "Cratchit Family", "Stave 3"]
    },
    {
        text: "Bob held his withered little hand in his",
        source: ["A Christmas Carol", "Cratchit Family", "Stave 3"]
    },
    {
        text: "Spirit of Tiny Tim, thy childish essence was from God",
        source: ["A Christmas Carol", "Scrooge", "Stave 4"]
    },
    {
        text: "It's likely to be a very cheap funeral",
        source: ["A Christmas Carol", "Businessmen", "Stave 4"]
    },
    {
        text: "He frightened everyone away from him when he was alive",
        source: ["A Christmas Carol", "Charwoman", "Stave 4"]
    },
    {
        text: "I am not the man I was",
        source: ["A Christmas Carol", "Scrooge's Transformation", "Stave 5"]
    },
    {
        text: "I will live in the Past, the Present, and the Future",
        source: ["A Christmas Carol", "Scrooge's Transformation", "Stave 5"]
    },
    {
        text: "Not the little prize turkey: the big one",
        source: ["A Christmas Carol", "Scrooge's Transformation", "Stave 5"]
    },
    {
        text: "I'll raise your salary",
        source: ["A Christmas Carol", "Scrooge's Transformation", "Stave 5"]
    },
    {
        text: "He was better than his word",
        source: ["A Christmas Carol", "Narrator", "Stave 5"]
    },

    // An Inspector Calls
    {
        text: "an impression of massiveness, solidity and purposefulness",
        source: ["An Inspector Calls", "Inspector", "Act 1"]
    },
    {
        text: "young women counting their pennies in the dingy little back bedroom",
        source: ["An Inspector Calls", "Inspector", "Act 1"]
    },
    {
        text: "it's better to ask for the earth than to take it",
        source: ["An Inspector Calls", "Inspector", "Act 1"]
    },
    {
        text: "Public men, Mr Birling, have responsibilities as well as privileges",
        source: ["An Inspector Calls", "Inspector", "Act 1"]
    },
    {
        text: "there are millions and millions and millions of Eva Smiths and John Smiths",
        source: ["An Inspector Calls", "Inspector", "Act 1"]
    },
    {
        text: "We don't live alone. We are members of one body. We are responsible for each other",
        source: ["An Inspector Calls", "Inspector", "Act 1"]
    },
    {
        text: "fire and blood and anguish",
        source: ["An Inspector Calls", "Inspector", "Act 1"]
    },
    {
        text: "a hard-headed practical man of business",
        source: ["An Inspector Calls", "Mr Birling", "Act 1"]
    },
    {
        text: "The Titanicâ€¦unsinkable, absolutely unsinkable",
        source: ["An Inspector Calls", "Mr Birling", "Act 1"]
    },
    {
        text: "you'd think everybody has to look after everybody elseâ€¦ like bees in a hive",
        source: ["An Inspector Calls", "Mr Birling", "Act 1"]
    },
    {
        text: "but these girls aren't cheap labour â€“ they're people",
        source: ["An Inspector Calls", "Sheila", "Act 1"]
    },
    {
        text: "I remember what he said, how he looked, and what he made me feel",
        source: ["An Inspector Calls", "Sheila", "Act 1"]
    },
    {
        text: "I know I'm to blame â€“ and I'm desperately sorry",
        source: ["An Inspector Calls", "Sheila", "Act 1"]
    },
    {
        text: "I suppose we're all nice people now",
        source: ["An Inspector Calls", "Sheila", "Act 1"]
    },
    {
        text: "You were the wonderful fairy prince",
        source: ["An Inspector Calls", "Sheila", "Act 1"]
    },
    {
        text: "she was young and pretty and warm hearted - and intensely grateful",
        source: ["An Inspector Calls", "Gerald", "Act 1"]
    },
    {
        text: "I don't suppose for a moment that we can understand why the girl committed suicide - girls of that class",
        source: ["An Inspector Calls", "Mrs Birling", "Act 1"]
    },
    {
        text: "Go and look for the father of the child. It's his responsibility",
        source: ["An Inspector Calls", "Mrs Birling", "Act 1"]
    },
    {
        text: "Why shouldn't they try for higher wages? We try for the highest possible prices",
        source: ["An Inspector Calls", "Eric", "Act 1"]
    },
    {
        text: "you killed her â€“ and the child she'd have had too",
        source: ["An Inspector Calls", "Eric", "Act 1"]
    },

    // Macbeth
    {
        text: "Fair is foul and foul is fair",
        source: ["Macbeth", "The Witches", "Act 1 Scene 1"]
    },
    {
        text: "Unseamed him from the nave to the chops",
        source: ["Macbeth", "The Captain", "Act 1 Scene 2"]
    },
    {
        text: "So foul and fair a day I have not seen",
        source: ["Macbeth", "Macbeth", "Act 1 Scene 3"]
    },
    {
        text: "Stars hide your fires; let not light see my black and deep desires",
        source: ["Macbeth", "Macbeth", "Act 1 Scene 4"]
    },
    {
        text: "Too full o' th' milk of human kindness",
        source: ["Macbeth", "Lady Macbeth", "Act 1 Scene 5"]
    },
    {
        text: "Unsex me here",
        source: ["Macbeth", "Lady Macbeth", "Act 1 Scene 5"]
    },
    {
        text: "Look like th' innocent flower, but be the serpent under't",
        source: ["Macbeth", "Lady Macbeth", "Act 1 Scene 5"]
    },
    {
        text: "vaulting ambition, which o'erleaps itself",
        source: ["Macbeth", "Macbeth", "Act 1 Scene 7"]
    },
    {
        text: "I dare do all that may become a man",
        source: ["Macbeth", "Macbeth", "Act 1 Scene 7"]
    },
    {
        text: "Screw your courage to the sticking place",
        source: ["Macbeth", "Lady Macbeth", "Act 1 Scene 7"]
    },
    {
        text: "Is this a dagger which I see before me",
        source: ["Macbeth", "Macbeth", "Act 2 Scene 1"]
    },
    {
        text: "Macbeth doth murder sleep",
        source: ["Macbeth", "Macbeth", "Act 2 Scene 2"]
    },
    {
        text: "will all great Neptune's ocean wash this blood clean from my hands",
        source: ["Macbeth", "Macbeth", "Act 2 Scene 2"]
    },
    {
        text: "A little water clears us of this deed",
        source: ["Macbeth", "Lady Macbeth", "Act 2 Scene 2"]
    },
    {
        text: "Where we are, there's daggers in men's smiles",
        source: ["Macbeth", "Donalbain", "Act 2 Scene 3"]
    },
    {
        text: "Our fears in Banquo stick deep",
        source: ["Macbeth", "Macbeth", "Act 3 Scene 1"]
    },
    {
        text: "O, full of scorpions is my mind, dear wife",
        source: ["Macbeth", "Macbeth", "Act 3 Scene 2"]
    },
    {
        text: "Never shake thy gory locks at me",
        source: ["Macbeth", "Macbeth", "Act 3 Scene 4"]
    },
    {
        text: "Blood will have blood",
        source: ["Macbeth", "Macbeth", "Act 3 Scene 4"]
    },
    {
        text: "I am in blood stepped so far",
        source: ["Macbeth", "Macbeth", "Act 3 Scene 4"]
    },
    {
        text: "By the pricking of my thumbs, something wicked this way comes",
        source: ["Macbeth", "The Witches", "Act 4 Scene 1"]
    },
    {
        text: "Dispute it like a man",
        source: ["Macbeth", "Malcolm", "Act 4 Scene 3"]
    },
    {
        text: "Out, damned spot! Out, I say",
        source: ["Macbeth", "Lady Macbeth", "Act 5 Scene 1"]
    },
    {
        text: "All the perfumes of Arabia will not sweeten this little hand",
        source: ["Macbeth", "Lady Macbeth", "Act 5 Scene 1"]
    },
    {
        text: "Out, out, brief candle",
        source: ["Macbeth", "Macbeth", "Act 5 Scene 5"]
    },
    {
        text: "Life's but a walking shadow",
        source: ["Macbeth", "Macbeth", "Act 5 Scene 5"]
    },
    {
        text: "a tale told by an idiot, full of sound and fury signifying nothing",
        source: ["Macbeth", "Macbeth", "Act 5 Scene 5"]
    },
    {
        text: "Macduff was from his mother's womb untimely ripped",
        source: ["Macbeth", "Macduff", "Act 5 Scene 8"]
    },
    {
        text: "this dead butcher and his fiend-like queen",
        source: ["Macbeth", "Malcolm", "Act 5 Scene 8"]
    },

    // Poetry
    {
        text: "Sneer of cold command",
        source: ["Poetry", "Ozymandias", ""]
    },
    {
        text: "Nothing beside remains",
        source: ["Poetry", "Ozymandias", ""]
    },
    {
        text: "colossal wreck, boundless and bare",
        source: ["Poetry", "Ozymandias", ""]
    },
    {
        text: "Look on my works ye Mighty and despair",
        source: ["Poetry", "Ozymandias", ""]
    },
    {
        text: "Mind-forged manacles",
        source: ["Poetry", "London", ""]
    },
    {
        text: "marks of weakness, marks of woe",
        source: ["Poetry", "London", ""]
    },
    {
        text: "hapless soldier's sigh / Runs in blood down palace walls",
        source: ["Poetry", "London", ""]
    },
    {
        text: "And blights with plagues the marriage hearse",
        source: ["Poetry", "London", ""]
    },
    {
        text: "a huge peak, black and huge",
        source: ["Poetry", "The Prelude", ""]
    },
    {
        text: "huge and mighty formsâ€¦ a trouble to my dreams",
        source: ["Poetry", "The Prelude", ""]
    },
    {
        text: "I gave commands; / Then all smiles stopped",
        source: ["Poetry", "My Last Duchess", ""]
    },
    {
        text: "my gift of a nine-hundred-years-old name",
        source: ["Poetry", "My Last Duchess", ""]
    },
    {
        text: "I choose / Never to stoop",
        source: ["Poetry", "My Last Duchess", ""]
    },
    {
        text: "jaws of deathâ€¦ mouth of hell",
        source: ["Poetry", "Charge of the Light Brigade", ""]
    },
    {
        text: "Theirs not to make reply, / Theirs not to reason why, / Theirs but to do and die",
        source: ["Poetry", "Charge of the Light Brigade", ""]
    },
    {
        text: "Honour the Light Brigade / Noble six hundred",
        source: ["Poetry", "Charge of the Light Brigade", ""]
    },
    {
        text: "Our brains ache, in the merciless iced east winds that knive us",
        source: ["Poetry", "Exposure", ""]
    },
    {
        text: "But nothing happens",
        source: ["Poetry", "Exposure", ""]
    },
    {
        text: "Dawn massing in the east her melancholy army",
        source: ["Poetry", "Exposure", ""]
    },
    {
        text: "We are prepared",
        source: ["Poetry", "Storm on the Island", ""]
    },
    {
        text: "spits like a tame cat / Turned savage",
        source: ["Poetry", "Storm on the Island", ""]
    },
    {
        text: "Strange, it is a huge nothing that we fear",
        source: ["Poetry", "Storm on the Island", ""]
    },
    {
        text: "Suddenly he awoke and was running",
        source: ["Poetry", "Bayonet Charge", ""]
    },
    {
        text: "King, honour, human dignity, etcetera / dropped like luxuries",
        source: ["Poetry", "Bayonet Charge", ""]
    },
    {
        text: "His terror's touchy dynamite",
        source: ["Poetry", "Bayonet Charge", ""]
    },
    {
        text: "every round rips through his life",
        source: ["Poetry", "Remains", ""]
    },
    {
        text: "End of story, except not really",
        source: ["Poetry", "Remains", ""]
    },
    {
        text: "his blood shadow stays",
        source: ["Poetry", "Remains", ""]
    },
    {
        text: "His bloody life in my bloody hands",
        source: ["Poetry", "Remains", ""]
    },
    {
        text: "spasms of paper red, disrupting a blockade",
        source: ["Poetry", "Poppies", ""]
    },
    {
        text: "released a song bird from its cage",
        source: ["Poetry", "Poppies", ""]
    },
    {
        text: "leaned against it like a wishbone",
        source: ["Poetry", "Poppies", ""]
    },
    {
        text: "spools of suffering set out in ordered rows",
        source: ["Poetry", "War Photographer", ""]
    },
    {
        text: "A hundred agonies in black and white",
        source: ["Poetry", "War Photographer", ""]
    },
    {
        text: "Between the bath and pre-lunch beers",
        source: ["Poetry", "War Photographer", ""]
    },
    {
        text: "Paper that lets the light shine through",
        source: ["Poetry", "Tissue", ""]
    },
    {
        text: "how easily they fall away on a sigh, a shift",
        source: ["Poetry", "Tissue", ""]
    },
    {
        text: "never meant to last",
        source: ["Poetry", "Tissue", ""]
    },
    {
        text: "I am branded by an impression of sunlight",
        source: ["Poetry", "The Emigree", ""]
    },
    {
        text: "time rolls its tanks",
        source: ["Poetry", "The Emigree", ""]
    },
    {
        text: "I have no passport, there's no way back at all",
        source: ["Poetry", "The Emigree", ""]
    },
    {
        text: "Bandage up me eye with my own history",
        source: ["Poetry", "Checking Out Me History", ""]
    },
    {
        text: "I carving out me identity",
        source: ["Poetry", "Checking Out Me History", ""]
    },
    {
        text: "dem tell me",
        source: ["Poetry", "Checking Out Me History", ""]
    },
    {
        text: "a one-way journey into history",
        source: ["Poetry", "Kamikaze", ""]
    },
    {
        text: "they treated him / as though he no longer existed",
        source: ["Poetry", "Kamikaze", ""]
    },
    {
        text: "he must have wondered which had been the better way to die",
        source: ["Poetry", "Kamikaze", ""]
    }
];
